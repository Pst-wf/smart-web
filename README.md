...
### 平台简介
* 采用前后端分离的模式，前端框架为：smart-web (基于 Vue、Ant Design Vue)，后端框架为：smart-boot (使用 Springboot、Mybatis-plus Security Oauth2 等技术栈)
...
9. 数据权限：配置菜单的可使用数据权限，在角色管理中进行勾选配置，支持7种数据权限模式（本人可见，本部门可见，本公司可见，指定用户可见，指定部门可见，拼接自定义SQL），灵活方便管理
