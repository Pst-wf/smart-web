...
router.beforeEach(async (to, from, next) => {
    ...
    if (to.meta.keepAlive) {
        nextTick(() => {
            useUserStore().pushKeepAliveIncludes(to.name);
        });
    }
    ...
    for (const e of menuList) {
        let route = {
            ...
            meta: {
                ...
                title: e.meta.title,
                // 菜单展示
                isLeaf: Boolean(!e.children),
                // 菜单图标展示
                icon: e.meta.antIcon,
                // 是否在菜单隐藏
                hideInMenu: e.meta.hideInMenu,
                // 页面是否keep-alive缓存
                keepAlive: e.meta.keepAlive,
                // 是否为外链
                frameFlag: Boolean(e.meta.href),
                // 外链地址
                frameUrl: e.meta.href,
                // 是否 rename了组件的名字
        ...
        if (e.meta.href) {
            ...
            let relativePath = `../views${e.path}/index.vue`;
            // // eslint-disable-next-line no-prototype-builtins
            route.component = modules[relativePath];
        }
        routerList.push(route);
        routerMap.set(e.name, route);
    }
