<template>
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="用户昵称" class="smart-query-form-item">
          <a-input style="width: 300px" v-model:value="queryForm.nickname" placeholder="请输入用户昵称" allowClear/>
        </a-form-item>
        <a-form-item label="联系电话" class="smart-query-form-item">
          <a-input style="width: 300px" v-model:value="queryForm.phone" placeholder="请输入联系电话" allowClear/>
        </a-form-item>
        <a-form-item label="启用状态" class="smart-query-form-item">
          <SmartEnumSelect width="300px" enum-name="STATUS_ENUM" v-model:value="queryForm.userStatus" placeholder="请选择状态" />
        </a-form-item>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      <a-row class="smart-table-btn-block">
        <div class="smart-table-operate-block">
          ...
          <a-button v-privilege="'frontUser:delete'" type="primary" danger @click="batchDelete" :disabled="!hasSelected">
            <template #icon>
              <DeleteOutlined />
            </template>
            批量删除
          </a-button>
          <a-button v-privilege="'frontUser:export'" type="primary" ghost @click="exportData">
            <template #icon>
              <ExportOutlined />
            </template>
            导出
          </a-button>
          <a-button v-privilege="'frontUser:import'" type="primary" danger ghost @click="openImportModal">
            <template #icon>
              <ImportOutlined />
            </template>
            导入
      ...
      >
        <template #bodyCell="{ text, record, column }">
          ...
          <template v-if="column.dataIndex === 'gender'">
            <div v-if="text">
              <a-tag :color="text === '1' ? 'blue' : 'red'">{{ text === '1' ? '男' : '女' }}</a-tag>
            </div>
          </template>
    ...
    <ImportData ref="importData" import-url="/business/frontUser/import" @downloadTemplate="downloadTemplate" @finish="query" />
  </div>
</template>
<script setup>
...
async function query() {
  try {
    ...
    let res = await frontUserApi.frontUserPage(queryForm);
...
function confirmBatchDelete(array) {
  ...
  async function requestBatchDelete(params) {
    SmartLoading.show();
    try {
      await frontUserApi.deleteFrontUser(params);
      // 清除选中
...
async function exportData() {
  try {
    ...
    let res = await frontUserApi.exportFrontUserData(params);
...
async function downloadTemplate() {
  try {
    tableLoading.value = true;
    let res = await frontUserApi.downloadFrontUserTemplate();
    message.success('操作成功');
    exportFile(res.data, res.headers);
  } catch (e) {
    smartSentry.captureError(e);
