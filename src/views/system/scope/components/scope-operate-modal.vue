    ...
    <a-form ref="formRef" :labelCol="{ span: 4 }" :labelWrap="true" :model="form" :rules="rules" :disabled="disabled">
      ...
      <a-form-item label="数据权限类型" name="scopeType">
        <a-select v-model:value="form.scopeType" placeholder="请选择数据权限类型" @change="scopeTypeChange">
          <a-select-option v-for="item in SCOPE_TYPE_ENUM" :key="item.value" :title="item.desc">{{ item.desc }} </a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item label="数据权限类名" name="scopeClass">
        <a-input v-model:value="form.scopeClass" placeholder="请输入数据权限类名" />
      </a-form-item>
      <a-form-item v-if="form.scopeType === '1' || form.scopeType === '2' || form.scopeType === '3'" label="数据权限字段" name="scopeField">
        <a-input v-model:value="form.scopeField" placeholder="请输入数据权限字段" />
      </a-form-item>
      <a-form-item v-if="form.scopeType === '6'" label="数据权限sql" name="scopeSql">
        <a-textarea :rows="3" v-model:value="form.scopeSql" placeholder="请输入数据权限sql" />
      </a-form-item>
      <a-form-item v-if="form.scopeType === '4'" label="可见范围" name="visibilityValue">
        <a-select
      ...
      <a-form-item v-if="form.scopeType === '5'" label="可见范围" name="visibilityValue">
        <a-tree-select
            v-model:value="form.visibilityValue"
            show-search
            multiple
...
const rules = {
  scopeName: [
    { required: true, message: '数据权限名称不能为空' },
    { max: 255, message: '长度不能超过255个字符', trigger: 'blur' },
  ],
  scopeType: [{ required: true, message: '数据权限类型不能为空' }],
  scopeClass: [
    { required: true, message: '数据权限类名不能为空' },
    { max: 500, message: '长度不能超过500个字符', trigger: 'blur' },
  ],
  scopeField: [{ max: 255, message: '长度不能超过255个字符', trigger: 'blur' }],
};
