...
async function getConfig() {
  ...
  if (res.data.export_length) {
    exportLength.value = Number(res.data.export_length);
  }
}
...
const updateEmailAsync = debounceAsync(
    (emailConfigKey) => {
...
<template>
  <a-form :labelCol="{ span: 4 }" :labelWrap="true" :model="map">
    <a-card title="框架设置" class="card-class" size="small" :bordered="false" :hoverable="false" :headStyle="{ borderBottom: 0 }">
      <a-row>
        <a-col :span="24">
          <a-form-item v-if="map.has_dept_post !== undefined" label="是否开启组织机构" name="has_dept_post">
            <a-switch
                @change="(val) => updateAsync('has_dept_post', val)"
                v-model:checked="map.has_dept_post"
                checked-value="1"
                un-checked-value="0"
                checked-children="开"
    ...
    <a-card title="认证设置" class="card-class" size="small" :bordered="false" :hoverable="false" :headStyle="{ borderBottom: 0 }">
      <a-row>
        <a-col :span="24">
          <a-form-item v-if="map.captcha_auth !== undefined" label="PC端是否开启验证码" name="captcha_auth">
            <a-switch
                @change="(val) => updateAsync('captcha_auth', val)"
                v-model:checked="map.captcha_auth"
                checked-value="1"
                un-checked-value="0"
                checked-children="开"
                un-checked-children="关"
      ...
      <a-row>
        <a-col :span="24">
          <a-form-item v-if="map.auto_sign_up !== undefined" label="开启会员注册" name="auto_sign_up">
            <a-switch
                @change="(val) => updateAsync('auto_sign_up', val)"
                v-model:checked="map.auto_sign_up"
                checked-value="1"
                un-checked-value="0"
                checked-children="开"
                un-checked-children="关"
      ...
      <a-row v-if="tokenMap.access_token_validity !== undefined">
        <a-col :span="12">
          <a-form-item label="AccessToken有效期" name="access_token_validity" :label-col="{ span: 8 }">
            <a-input-number
                v-model:value="tokenMap.access_token_validity"
                placeholder="请输入AccessToken有效期"
                allowClear
                style="width: 100%"
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateTokenAsync('access_token_validity', tokenMap.access_token_validity)"> 保存 </a-button>
        </a-col>
      </a-row>
      <a-row v-if="tokenMap.refresh_token_validity !== undefined">
        <a-col :span="12">
          <a-form-item label="RefreshToken有效期" name="refresh_token_validity" :label-col="{ span: 8 }">
            <a-input-number
                v-model:value="tokenMap.refresh_token_validity"
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateTokenAsync('refresh_token_validity', tokenMap.refresh_token_validity)"> 保存 </a-button>
        </a-col>
      </a-row>
    </a-card>
    <a-card title="登陆失败锁定设置" class="card-class" size="small" :bordered="false" :hoverable="false" :headStyle="{ borderBottom: 0 }">
      <a-row>
        <a-col :span="24">
          <a-form-item v-if="map.sys_login_fail_lock_status !== undefined" label="开启状态" name="sys_login_fail_lock_status">
            <a-switch
                @change="(val) => updateAsync('sys_login_fail_lock_status', val)"
                v-model:checked="map.sys_login_fail_lock_status"
                checked-value="1"
                un-checked-value="0"
                checked-children="开"
                un-checked-children="关"
      ...
      <a-row v-if="map.sys_login_fail_lock_status === '1'">
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateAsync('sys_login_fail_check_cycle', lockCycle)"> 保存 </a-button>
        </a-col>
      </a-row>
      <a-row v-if="map.sys_login_fail_lock_status === '1'">
        <a-col :span="12">
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateAsync('sys_login_fail_times', lockTimes)"> 保存 </a-button>
        </a-col>
      </a-row>
      <a-row v-if="map.sys_login_fail_lock_status === '1'">
        <a-col :span="12">
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateAsync('sys_login_fail_lock_duration', lockDuration)"> 保存 </a-button>
        </a-col>
      </a-row>
    </a-card>
    <a-card title="导出设置" class="card-class" size="small" :bordered="false" :hoverable="false" :headStyle="{ borderBottom: 0 }">
      <a-row v-if="map.export_length !== undefined">
        <a-col :span="12">
          <a-form-item label="一次可以导出数据条数" name="exportLength" :label-col="{ span: 8 }">
            <a-input-number
                v-model:value="exportLength"
                placeholder="请输入导出数据条数"
                allowClear
                style="width: 100%"
                :formatter="(value) => `${value}`"
                :precision="0"
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateAsync('export_length', exportLength)">保存</a-button>
        </a-col>
      </a-row>
    </a-card>
    <a-card title="文档设置" class="card-class" size="small" :bordered="false" :hoverable="false" :headStyle="{ borderBottom: 0 }">
      ...
      <a-row v-if="map.file_document_callback_url !== undefined">
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateAsync('file_document_callback_url', map.file_document_callback_url)"> 保存 </a-button>
        </a-col>
      </a-row>
      <a-row v-if="map.file_document_local_url !== undefined">
        ...
        <a-col :span="8" style="margin-left: 20px">
          <a-button type="primary" @click="updateAsync('file_document_local_url', map.file_document_local_url)"> 保存 </a-button>
        </a-col>
      </a-row>
    </a-card>
    <a-card title="微信设置" class="card-class" size="small" :bordered="false" :hoverable="false" :headStyle="{ borderBottom: 0 }">
    ...
    <a-card
        v-if="map.sys_email_config !== undefined"
        title="邮箱设置"
        class="card-class"
        size="small"
        :bordered="false"
