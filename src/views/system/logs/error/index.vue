  ...
  <div>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      ...
      <a-table
          ...
          rowKey="id"
          :pagination="false"
          class="error-log-class"
          @change="(pagination, filters, sorter) => $sorterChange(() => query(), sorter, queryForm, columns)"
      >
        <template #expandedRowRender="{ record }">
          <ul>
            <li style="margin-left: 20px;list-style-type: none;color: red; font-weight: bold">{{record.exceptionMessage}}</li>
            <li v-for="(item,index) in record.stacktraceList" :key="index" style="margin-left: 20px;list-style-type: none">
              {{getHtml(item)}}
            </li>
          </ul>
        </template>
...
import {errorLogApi} from "/@/api/system/error-log-api.js";
...
async function query() {
  try {
    ...
    let res = await errorLogApi.errorLogPage(params);
...
function confirmBatchDelete(array) {
  ...
  async function requestBatchDelete(params) {
    SmartLoading.show();
    try {
      await errorLogApi.deleteErrorLog(params);
      // 清除选中
...
function getHtml(item){
  let value = `at ${item.className}.${item.methodName}`;
  if (item.nativeMethod) {
    value += '(Native Method)';
  } else if (item.fileName !== null && item.lineNumber >= 0) {
    value += `(${item.fileName}:${item.lineNumber})`;
