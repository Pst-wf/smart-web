    ...
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        ...
        <a-form-item label="操作类型" class="smart-query-form-item">
          <SmartEnumSelect width="300px" enum-name="OPERATE_LOG_TYPE_ENUM" v-model:value="queryForm.logType" placeholder="请选择操作类型"/>
        </a-form-item>
      ...
      >
        <template #bodyCell="{ text, record, column }">
          <template v-if="column.dataIndex === 'logType'">
            <a-tag :color="text === 'add' ? 'green' : (text === 'update' ? 'blue' : 'red')">{{ $smartEnumPlugin.getDescByValue('OPERATE_LOG_TYPE_ENUM', text) }}</a-tag>
          </template>
...
import {operateLogApi} from "/@/api/system/operate-log-api.js";
...
async function query() {
  try {
    ...
    let res = await operateLogApi.operateLogPage(params);
...
function confirmBatchDelete(array) {
  ...
  async function requestBatchDelete(params) {
    SmartLoading.show();
    try {
      await operateLogApi.deleteOperateLog(params);
      // 清除选中
...
function showDrawer(rowData) {
  operateModal.value.showDrawer(rowData);
}
</script>
