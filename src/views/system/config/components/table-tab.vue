<template>
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="键" class="smart-query-form-item">
          <a-input style="width: 300px" v-model:value="queryForm.configKey" placeholder="请输入键"/>
        </a-form-item>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      <a-row class="smart-table-btn-block">
        <div class="smart-table-operate-block">
          <a-button v-privilege="'config:add'" type="primary" @click="showDrawer">
            <template #icon>
              <PlusOutlined/>
            </template>
            新增
          </a-button>
          <a-button v-privilege="'config:delete'" type="primary" danger @click="batchDelete" :disabled="!hasSelected">
            <template #icon>
              <DeleteOutlined/>
            </template>
            批量删除
        ...
        <template #bodyCell="{ text, record, column }">
          ...
          <template v-if="column.dataIndex === 'operate'">
            <div class="smart-table-operate">
              ...
              <a-button v-privilege="'config:update'" type="link" size="small" @click="showDrawer(record)">编辑</a-button>
              <a-button v-privilege="'config:delete'" danger type="link" @click="singleDelete(record)">删除</a-button>
            </div>
          </template>
        </template>
      </a-table>
    ...
    <OperateModal ref="operateModal" :is-system="props.isSystem" @reloadList="query"/>
  </div>
</template>
<script setup>
...
async function query() {
  try {
    ...
    let res = await configApi.configPage(queryForm);
...
function confirmBatchDelete(array) {
  ...
  async function requestBatchDelete(params) {
    SmartLoading.show();
    try {
      await configApi.deleteConfig(params);
      // 清除选中
