  ...
  <div>
    ...
    >
      <div>
        ...
        <a-list :split="false">
          <a-list-item v-for="item in drawerContent.arr1" :key="item.id"> {{ item.label }} ： {{ item.value }} </a-list-item>
        </a-list>
        <a-divider />
        <h3 class="drawer-title">触发日志</h3>
        <a-list :split="false">
          <a-list-item v-for="item in drawerContent.arr2" :key="item.id"> {{ item.label }} ： {{ item.value }} </a-list-item>
        </a-list>
      </div>
    </a-drawer>
    <a-modal :open="modalVisible" title="执行日志" :maskClosable="true" :width="960" :get-container="SmartLoading.spin" @cancel="closeHandleMsg">
