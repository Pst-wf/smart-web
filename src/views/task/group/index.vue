    ...
    <a-card size="small" :bordered="false" :hoverable="true">
      <a-row class="smart-table-btn-block">
        <div class="smart-table-operate-block">
          <a-button v-privilege="'jobGroup:add'" type="primary" @click="showDrawer">
            <template #icon>
              <PlusOutlined/>
            </template>
            新增
          </a-button>
          <a-button v-privilege="'jobGroup:delete'" type="primary" danger @click="batchDelete" :disabled="!hasSelected">
            <template #icon>
              <DeleteOutlined/>
            </template>
            批量删除
      ...
      >
        <template #bodyCell="{ text, record, column }">
          <template v-if="column.dataIndex === 'addressType'">
            <a-tag :color="text === '1' ? 'blue' : 'green' ">{{ $smartEnumPlugin.getDescByValue('ADDRESS_TYPE_ENUM', text) }}</a-tag>
          </template>
          <template v-if="column.dataIndex === 'onlineStatus'">
            <a-tag :color="text === '1' ? 'blue' : 'red' ">{{ $smartEnumPlugin.getDescByValue('ONLINE_STATUS_ENUM', text) }}</a-tag>
          </template>
          <template v-if="column.dataIndex === 'operate'">
            <div class="smart-table-operate">
              <a-button type="link" size="small" @click="showDrawer(record, true)">查看</a-button>
              <a-button v-privilege="'jobGroup:update'" type="link" size="small" @click="showDrawer(record)">编辑</a-button>
              <a-button v-privilege="'jobGroup:delete'" danger type="link" @click="singleDelete(record)">删除</a-button>
            </div>
          </template>
        </template>
      </a-table>
...
async function query() {
  try {
    ...
    let res = await taskGroupApi.taskGroupPage(queryForm);
...
function confirmBatchDelete(array) {
  ...
  async function requestBatchDelete(params) {
    SmartLoading.show();
    try {
      await taskGroupApi.deleteTaskGroup(params);
      // 清除选中
