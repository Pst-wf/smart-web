  ...
  >
    <a-form ref="formRef" :labelCol="{ span: 4 }" :labelWrap="true" :model="form" :rules="rules" :disabled="disabled">
      ...
      <a-form-item label="执行器地址列表" name="addressList">
        <a-textarea :auto-size="{ minRows: 2, maxRows: 4 }" v-model:value="form.addressList" placeholder="执行器地址列表" />
      </a-form-item>
    </a-form>
    <div class="footer">
...
<script setup>
  ...
  const rules = {
    appName: [{ required: true, message: '执行器不能为空' }],
    title: [{ required: true, message: '执行器名称不能为空' }],
  };
