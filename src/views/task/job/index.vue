<template>
  <div>
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="执行器" class="smart-query-form-item">
          <a-input style="width: 300px" v-model:value="queryForm.groupName" placeholder="请输入执行器名称"/>
        </a-form-item>
        <a-form-item label="任务描述" class="smart-query-form-item">
          <a-input style="width: 300px" v-model:value="queryForm.jobDesc" placeholder="请输入任务描述"/>
        </a-form-item>
        ...
        <template #bodyCell="{ text, record, column }">
          ...
          <template v-if="column.dataIndex === 'triggerStatus'">
            <a-tag :color="text === 1 ? 'blue' : 'red' ">{{
                $smartEnumPlugin.getDescByValue('TRIGGER_STATUS_ENUM', text)
              }}
            </a-tag>
          </template>
          <template v-if="column.dataIndex === 'operate'">
            <div class="smart-table-operate">
              ...
              <a-button v-privilege="'job:update'" type="link" size="small" @click="showDrawer(record,'edit')">编辑
              </a-button>
              ...
              <a-dropdown placement="bottomLeft">
                ...
                <template #overlay>
                  <a-menu>
                    ...
                    <a-menu-item>
                      <a-button type="link" style="color: #1e1e1e" size="small" @click="viewLogs(record)">查看日志
                      </a-button>
                    </a-menu-item>
                    <a-menu-item>
                      <a-button type="link" style="color: #1e1e1e" size="small" @click="showModal(record)">注册节点
                      </a-button>
                    </a-menu-item>
                    <a-menu-item>
                      <a-button v-privilege="'job:start'" type="link" style="color: #1e1e1e" size="small" @click="start(record)">启动
                      </a-button>
                    </a-menu-item>
                    <a-menu-item>
                      <a-button v-privilege="'job:stop'" type="link" style="color: #1e1e1e" size="small" @click="stop(record)">停止
                      </a-button>
                    </a-menu-item>
                    <a-menu-item>
    ...
    <a-modal v-model:open="open" title="注册节点">
