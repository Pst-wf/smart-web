  ...
  <div>
    <a-card size="small" :bordered="false" :hoverable="true">
      ...
      <a-table
          ...
          rowKey="id"
          :pagination="false"
          @change="(pagination, filters, sorter) => $sorterChange(() => query(), sorter, queryForm, draftColumns)"
      >
      >
        <template #bodyCell="{ text, record, column }">
          ...
          <template v-if="column.dataIndex === 'operate'">
            <div class="smart-table-operate">
              <a-button v-privilege="'file:download'" type="link" size="small" @click="edit(record)">编辑
              </a-button>
            </div>
          </template>
        </template>
...
import {draftColumns} from './columns.js';
...
function resetQuery() {
  ...
  draftColumns.value.forEach(item => item.sortOrder = null)
  query();
}
...
async function toDetail(data) {
  ...
  messageDetail.value.showModal(res.data, 'draft')
}
function edit(row){
  emit('callback', row)
}
