  ...
  async function customRequest(file) {
    try {
      ...
      if (result.uploadType === '0') {
        resUrl = result.fileKey;
      } else {
        resUrl = result.filePath;
      }
...
<template>
  <a-modal :open="visible" title="修改头像" :maskClosable="true" :width="900" centered :get-container="SmartLoading.spin" @cancel="onClose">
    <a-row>
      <a-col :span="12">
