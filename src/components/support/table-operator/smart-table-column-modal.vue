<template>
  <a-modal :width="700" :open="visible" title="设置列" :destroyOnClose="true" :closable="true" :get-container="SmartLoading.spin" @cancel="hide">
      ...
      <template #bodyCell="{ text, record, index, column }">
        <template v-if="column.dataIndex === 'title'">
          <a-button type="text" :class="record.fixed ? '' : 'handle'" size="small" style="width: 100%; text-align: left">
            <template #icon v-if="!record.fixed">
              <drag-outlined v-if="!record.fixed" />
              <pushpin-outlined v-if="record.fixed" />
            </template>
            {{ text }}
          </a-button>
        </template>
        <template v-if="column.dataIndex === 'width'">
          <a-input-number v-model:value="record.width" style="width: 90px; margin-left: 10px; margin-right: 3px" size="small" />
          px
        </template>
    ...
    <template #footer>
      <a-button key="back" @click="hide">关闭</a-button>
...
async function getUserTableColumns(tableId, columns) {
  ...
  //   SmartLoading.hide();
  // }
  const setting = localRead(tableId + '__columns_setting');
  if (setting) {
    userTableColumnArray = JSON.parse(setting);
  }
  //根据前端列和后端列构建新的列数据
