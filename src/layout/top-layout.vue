    ...
    <a-layout-content id="smartAdminLayoutContent" class="admin-layout-content">
      ...
      <div v-show="!iframeNotKeepAlivePageFlag && keepAliveIframePages.every((e) => route.name !== e.name)">
        <router-view v-slot="{ Component }">
          <transition :name="animateMode" mode="out-in">
...
<script setup>
  ...
  const background = computed(() => (theme.value === 'light' ? themeMode.light.sideColor : themeMode.dark.sideColor));
  //是否显示标签页
  ...
  // 动画效果
  const animateMode = computed(() => useAppConfigStore().$state.animateMode);
  // 多余高度
  ...
  onMounted(() => {
    if (watermarkFlag.value) {
      watermark.set('smartAdminLayoutContent', useUserStore().nickname);
    } else {
      watermark.clear();
    }
  });
