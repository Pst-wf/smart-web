  ...
  <a-row style="border-bottom: 1px solid #eeeeee; position: relative" v-show="pageTagFlag">
    <a-dropdown :trigger="['contextmenu']">
      <div class="smart-page-tag">
        <a-tabs style="width: 100%" :tab-position="mode" v-model:activeKey="selectedKey" size="small" @tabClick="selectTab" class="tabs-class">
...
<script setup>
  ...
  function closeTags(items) {
    ...
    if (items) {
      ...
      // 关闭其他tag不做处理 直接调用closeTagNav
      items.forEach((item) => useUserStore().closeTagNav(item ? item.menuName : null, false));
    }
  }
