<template>
  <a-modal :open="visible" :maskClosable="true" :width="600" :closable="false" @cancel="onClose"
           :body-style="{padding: '0 24px'}" class="modal-class">
        ...
        <template #renderItem="{ item }">
          <a-list-item class="list-item-class" :style="itemStyle" @click="jump(item)">
            <a-flex gap="middle" justify-content="space-between" style="width: 100%">
              <div>
                <component v-if="item.meta.icon" :is="$antIcons[item.meta.icon]" style="font-size: 14px"
