<template>
  <a-drawer :title="$t('setting.title')" placement="right" :open="visible" :get-container="SmartLoading.spin" @close="close">
    <a-form layout="horizontal" :label-col="{ span: 8 }">
      ...
      <a-form-item :label="$t('setting.animateMode')">
        <a-select v-model:value="formState.animateMode" @change="changeAnimateMode" style="width: 120px">
          <a-select-option value="fade">淡入淡出</a-select-option>
          <a-select-option value="fade-slide">滑动</a-select-option>
          <a-select-option value="fade-bottom">底部消退</a-select-option>
          <a-select-option value="fade-scale">缩放消退</a-select-option>
          <a-select-option value="zoom-fade">渐变</a-select-option>
          <a-select-option value="zoom-out">闪现</a-select-option>
      ...
      <a-form-item :label="$t('setting.tableTextAlign')">
        <a-radio-group v-model:value="formState.tableTextAlign" button-style="solid" @change="changeTableTextAlign">
    ...
    <div class="footer">
      ...
      <a-button style="margin-right: 8px" type="primary" @click="save">保存配置</a-button>
...
<script setup>
  ...
  async function save() {
    ...
    localSave(localStorageKeyConst.APP_CONFIG, JSON.stringify(form));
    await userConfigApi.saveUserConfig(form);
  ...
  async function reset() {
    ...
    await userConfigApi.saveUserConfig(formState);
  }
  ...
  let formValue = {
    ...
    // 表格布局
    tableTextAlign: appConfigStore.tableTextAlign,
    // 动画
    animateMode: appConfigStore.animateMode,
  };
  ...
  function changeTableTextAlign(e) {
    appConfigStore.$patch({
      tableTextAlign: e.target.value,
    });
  }
  function changeAnimateMode(value) {
    appConfigStore.$patch({
      animateMode: value,
    });
  }
